...
但实际上，我们可能会遇到需要删除黑色节点的情况，该操作对应 2-3-4 树中删除 2-node 的操作，这将会破坏树的平衡性，因此我们需要保证我们所删除的节点是红色的。
...
实际的实现思路，根据兄弟节点的情况，可分为 2 种操作：

1. 兄弟节点也是 2-node，那么需要将父节点中分离一个节点出来，和兄弟节点一起形成一个 4-node；
2. 兄弟节点是 3-node 或者 4-node，那么只需要向兄弟节点借一个节点形成 3-node。

相关图示如下（该图来自 Sedgewick 的 PPT 第 55 页)：



操作 1，由于是自顶向下进行查找和合并，因此除了根节点，父节点一定是一个 3-node 或者 4-node。如果根节点是一个 2-node 怎么办

*这一方法与大部分文章所使用的方法并不同，因为它不会去对各种情况去进行区分，并且也不会出现遗漏的修复或者实际不存在的情况。*
