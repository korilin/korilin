...
**因为根据红黑树的性质 5，祖父节点下各个路径的黑色节点数都是相同的，因此叔叔节点只能是空节点或红色节点，不能是实际存在的黑色节点。但由于性质 4 规定空节点颜色也是黑色，有些文章可能描述为叔叔节点是黑色，但该描述所表示的节点应当是一个空节点。**
...
该方法的实现思路，根据兄弟节点的情况，可分为 2 种操作：

1. **combine siblings**：兄弟节点也是 2-node，那么需要将父节点中分离一个节点出来，和兄弟节点一起形成一个 4-node；
2. **borrow from sibling**：兄弟节点是 3-node 或者 4-node，那么只需要向兄弟节点借一个节点形成 3-node。

相关图示如下（该图来自 Sedgewick 的 PPT 第 55 页）：

![LLRB-RedBlack-55](./LLRB-RedBlack-55.jpg)

操作 1，由于是自顶向下进行查找和合并，因此除了根节点，父节点一定是一个 3-node 或者 4-node。如果根节点是一个 2-node，那么可以通过下方的两种方式解决：

1. 根节点的两个子节点也是 2-node：和子节点合并成 4-node，由于该操作是根节点进行，因此不会破坏树整体的平衡；
2. 根节点有一个子节点不是 2-node：向该节点借一个节点来形成 3-node，该方式在红黑树中可使用旋转来完成。

通过上面两种方式解决根节点是 2-node 的问题，即容易理解，也不会出现需要逐层合并的情况，实现起来比较方便。

### 在红黑树中的实现方式

上面 4 种操作对应到红黑树中，操作流程如下图所示：

![2-node-root-dealwith](./2-node-root-dealwith.jpg)

![combine-siblings](./combine-siblings.jpg)

Sibling 的操作比较麻烦，因为兄弟节点代表的 4-node 大小要比父节点小，因此当前节点并不是直接在兄弟节点中取一个进行合并，而是在父节点中取一个节点进行合并，父节点再从兄弟节点中拿一个进行补充。
...
对于兄弟节点是 3-node 的情况，也是将父节点变成一个 4-node，当兄弟节点的红色子节点与兄弟节点和父节点在同一斜线上时，只需要交换兄弟节点和红色子节点的颜色，如果不在同一斜线，就和 4-node 一样进行一次旋转操作。
